...
/*******************************************************************************
 * Command Line Interface Argument Parser for JMilter.
 * 
...
public class InfoMilterCLIArgsParser {
	...
	protected static InfoMilterCLIArgsParserBean readArgs(InfoMilterCLIArgsParserBean argsBean, String[] args)
		...
		final String USAGE = "[-i <IPv4-Address to listen>] [-p <Port to listen>] [-l <TCP-Log-Level>] [-h] [-v] [-d]";
		...
		options.addOption("l", "logging", true,
				"Enables the TCP-Logging for JMilter with the specified log level (INFO, WARN, ERROR, TRACE or DEBUG)");
		...
		if (cmd.hasOption("l")) {
			...
			if (cmd.getOptionValue("l").equals("INFO") || cmd.getOptionValue("l").equals("WARN")
					|| cmd.getOptionValue("l").equals("ERROR") || cmd.getOptionValue("l").equals("TRACE")
					|| cmd.getOptionValue("l").equals("DEBUG")) {
				argsBean.setTcpLoggingEnabled(true);
				argsBean.setTcpLogLevel(cmd.getOptionValue("l"));
			} else {
				new HelpFormatter().printHelp(USAGE, HEADER, options, FOOTER);
				throw new InfoMilterCLIArgParserException(
						"Parameter -l,--logging <TCP-Logging Log-Level> is NOT a valid! (Possible values: INFO, WARN, ERROR, TRACE or DEBUG)");
			}

		} else {
			argsBean.setTcpLoggingEnabled(false);
			argsBean.setTcpLogLevel("DEBUG");
		}
